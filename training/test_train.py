...
class SkyworkDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        from modelscope.msdatasets import MsDataset
        ds = MsDataset.load(
            "swift/SkyPile-150B",  # 注意！这里是 skywork，不是 swift
            split="train",
...
def main():
    ...
    # )
    # dataset = FineWebEduDataset(encoding, seq_len, dataset_subset, rank, world_size)
    loader = DataLoader(dataset, batch_size=micro_batch, num_workers=0, pin_memory=False)
