...
class MoEFFN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        scores = F.softmax(logits, dim=-1)          # B * T * n_exports
        _, topk_idx = (logits + self.router_bias).topk(self.topk, dim=-1)   # B * T * self.topk
        topk_scores = scores.gather(-1, topk_idx)   # B * T * self.topk
        topk_scores = topk_scores / topk_scores.sum(dim=-1, keepdim=True)  # renorm    B * T * self.topk
