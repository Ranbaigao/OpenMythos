...
print(f"[OpenMythos] flash-attn available: {_HAS_FLASH_ATTN}")
...
class MoEFFN(nn.Module):
    def __init__(self, cfg: MythosConfig):
        ...
        # =================================================================
        # 🚀 论文 ESFT-Token 核心：新增专家被选中的计数器
        # =================================================================
        self.register_buffer("expert_selection_counts", torch.zeros(cfg.n_experts))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # =================================================================
        # 🚀 ESFT Profiling 探针：如果开启，则静默统计 Token 去向
        # =================================================================
        if getattr(self, "profile_mode", False):
            with torch.no_grad():
                idx_flat = topk_idx.flatten()
                counts = torch.bincount(idx_flat, minlength=self.n_experts)
                self.expert_selection_counts += counts.to(self.expert_selection_counts.device)

        # ... (下方保留你之前优化过的高效 Grouped Dispatch 代码，保持不变) ...
        # topk_scores = ...
...
class OpenMythos(nn.Module):
    ...
    def generate(
        ...
        Args:
            ...
            repetition_penalty -- penalty for tokens already present in the
                sequence (1.0 = disabled, >1.0 discourages repetition)
        ...
        for step in range(max_new_tokens):
            ...
            if repetition_penalty != 1.0:
                # HF 风格重复惩罚：已出现过的 token，正 logit 除以惩罚系数，负 logit 乘以它
                prev_logits = logits.gather(1, input_ids)
                prev_logits = torch.where(
                    prev_logits > 0, prev_logits / repetition_penalty, prev_logits * repetition_penalty
                )
                logits.scatter_(1, input_ids, prev_logits)
            if top_k > 0:
